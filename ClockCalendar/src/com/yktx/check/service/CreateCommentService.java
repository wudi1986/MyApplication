...
public class CreateCommentService extends Service{
		...
		try {
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				...
				for(int i = 0; i<message.length(); i++){
					...
					Gson gson = new Gson();
					CommentsBean bean =  gson.fromJson(str, CommentsBean.class);
					beans.add(bean);
				}
				serviceListener.getJOSNdataSuccess(beans, retcode , Contanst.CREATECOMMENT);
			} else {
				String errmsg = (String) result.get("message");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.CREATECOMMENT);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.CREATECOMMENT);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.CREATECOMMENT);
