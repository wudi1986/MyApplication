...
public class TaskGetImageService extends Service{

	public TaskGetImageService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		// TODO Auto-generated constructor stub
		this.url=UrlParams.IP+Contanst.HTTP_TASK_GETIMAGE
				+urlParams;
...
//				String str = result.getString("message");
				ArrayList<TaskGetImageBean> list = new ArrayList<TaskGetImageBean>();
				JSONArray message = result.getJSONArray("message");
				for(int j = 0; j < message.length(); j++){
					String str = message.getString(j);
					Gson gson = new Gson();
					TaskGetImageBean bean = gson.fromJson(str, TaskGetImageBean.class);
					list.add(bean);
				}
				serviceListener.getJOSNdataSuccess(list, retcode , Contanst.TASK_GETIMAGE);
			} else {
				String errmsg = (String) result.get("message");
				Tools.getLog(Tools.i, "aaa", "errmsg = "+errmsg);
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.TASK_GETIMAGE);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.TASK_GETIMAGE);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.TASK_GETIMAGE);
