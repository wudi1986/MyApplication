...
public class BuildingGetTopPointUserService extends Service{

	public BuildingGetTopPointUserService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		// TODO Auto-generated constructor stub
		this.url = UrlParams.IP + Contanst.HTTP_BUILDING_GETTOPPOINTUSER+urlParams;
...
//				String str = result.getString("message");
				ArrayList<TopPointUserBean> beans = new ArrayList<TopPointUserBean>();
				
				JSONArray message = result.getJSONArray("message");
				for(int j = 0; j < message.length(); j++){
					String str = message.getString(j);
					Gson gson = new Gson();
					TopPointUserBean bean = gson.fromJson(str, TopPointUserBean.class);
					beans.add(bean);
				}
				serviceListener.getJOSNdataSuccess(beans, retcode , Contanst.BUILDING_GETTOPPOINTUSER);
			} else {
				String errmsg = (String) result.get("message");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.BUILDING_GETTOPPOINTUSER);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.BUILDING_GETTOPPOINTUSER);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.BUILDING_GETTOPPOINTUSER);
