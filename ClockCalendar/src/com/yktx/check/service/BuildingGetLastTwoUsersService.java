...
public class BuildingGetLastTwoUsersService extends Service{

	public BuildingGetLastTwoUsersService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		// TODO Auto-generated constructor stub
		this.url = UrlParams.IP + Contanst.HTTP_BUILDING_GETLASTTWOUSERS+urlParams;
	...
	void httpSuccess(String reponse) {
		...
		try {
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				...
				Gson gson = new Gson();
				LastTwoUsersBean lastTwoUsersBean = gson.fromJson(str, LastTwoUsersBean.class);
				
				serviceListener.getJOSNdataSuccess(lastTwoUsersBean, retcode , Contanst.BUILDING_GETLASTTWOUSERS);
			} else {
				String errmsg = (String) result.get("message");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.BUILDING_GETLASTTWOUSERS);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.BUILDING_GETLASTTWOUSERS);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.BUILDING_GETLASTTWOUSERS);
