...
public class GetLastImageTodayService extends Service{

	public GetLastImageTodayService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		// TODO Auto-generated constructor stub
		this.url = UrlParams.IP + Contanst.HTTP_GET_LAST_IMAGE_TODAY+urlParams;
	...
	void httpSuccess(String reponse) {
		...
		try {
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				String imageSource = result.getString("imageSource");
				String str = result.getString("message");
				serviceListener.getJOSNdataSuccess(str+","+imageSource, retcode , Contanst.GET_LAST_IMAGE_TODAY);
			} else {
				String errmsg = (String) result.get("message");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.GET_LAST_IMAGE_TODAY);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.GET_LAST_IMAGE_TODAY);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.GET_LAST_IMAGE_TODAY);
