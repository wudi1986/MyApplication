...
public class UserGetBuildingFollowingService extends Service{

	public UserGetBuildingFollowingService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		// TODO Auto-generated constructor stub
		this.url = UrlParams.IP + Contanst.HTTP_USER_GETBUILDINGFOLLOWING+urlParams;
	...
	void httpSuccess(String reponse) {
		...
		try {
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				...
				Gson gson = new Gson();
				ByTaskIdBean byTaskIdBean = gson.fromJson(str, ByTaskIdBean.class);
				serviceListener.getJOSNdataSuccess(byTaskIdBean, retcode , Contanst.USER_GETBUILDINGFOLLOWING);
			} else {
				String errmsg = (String) result.get("message");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.USER_GETBUILDINGFOLLOWING);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.USER_GETBUILDINGFOLLOWING);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.USER_GETBUILDINGFOLLOWING);
