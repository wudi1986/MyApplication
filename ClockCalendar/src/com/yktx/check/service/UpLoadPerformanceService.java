...
public class UpLoadPerformanceService extends Service{
	...
	void httpSuccess(String reponse) {
		...
		try {
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				...
				serviceListener.getJOSNdataSuccess(message, retcode , Contanst.UPLOADPERFORMANCE);
			} else {
				String errmsg = (String) result.get("message");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.UPLOADPERFORMANCE);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.UPLOADPERFORMANCE);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.UPLOADPERFORMANCE);
