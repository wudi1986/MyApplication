...
public class GetCategoryService extends Service{
	...
	public GetCategoryService(String requestType, Hashtable<String, String> params,
			String urlParams, ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		// TODO Auto-generated constructor stub
		this.url = UrlParams.IP + Contanst.HTTP_GET_CATEGORY;
	...
	void httpSuccess(String reponse) {
		...
		try {
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				...
				ArrayList<GetCategoryBean> list = new ArrayList<GetCategoryBean>();
				JSONArray message = result.getJSONArray("message");
				for (int j = 0; j < message.length(); j++) {
					String str = message.getString(j);
					Gson gson = new Gson();
					GetCategoryBean bean = gson.fromJson(str, GetCategoryBean.class);
					list.add(bean);
				}
				serviceListener.getJOSNdataSuccess(list, retcode , Contanst.GET_CATEGORY);
			} else {
				String errmsg = (String) result.get("message");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.GET_CATEGORY);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.GET_CATEGORY);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.GET_CATEGORY);
