...
public class UserSearchService extends Service{

	public UserSearchService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		// TODO Auto-generated constructor stub
		this.url = UrlParams.IP + Contanst.HTTP_USER_SEARCH;
	...
	void httpSuccess(String reponse) {
		...
		try {
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				...
				serviceListener.getJOSNdataSuccess(list, retcode , Contanst.USER_SEARCH);
			} else {
				String errmsg = (String) result.get("message");
				Tools.getLog(Tools.i, "aaa", "errmsg = "+errmsg);
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.USER_SEARCH);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.USER_SEARCH);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.USER_SEARCH);
