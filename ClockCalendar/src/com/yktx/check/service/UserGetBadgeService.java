...
public class UserGetBadgeService extends Service{

	public UserGetBadgeService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		// TODO Auto-generated constructor stub
		this.url = UrlParams.IP + Contanst.HTTP_USER_GETBADGE+urlParams;
...
//				String str = result.getString("message");
				ArrayList<MedalBean> list = new ArrayList<MedalBean>();
				JSONArray message = result.getJSONArray("message");
				for(int j = 0; j < message.length(); j++){
					String str = message.getString(j);
					Gson gson = new Gson();
					MedalBean bean = gson.fromJson(str, MedalBean.class);
					list.add(bean);
				}
				serviceListener.getJOSNdataSuccess(list, retcode , Contanst.USER_GETBADGE);
			} else {
				String errmsg = (String) result.get("message");
				Tools.getLog(Tools.i, "aaa", "errmsg = "+errmsg);
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.USER_GETBADGE);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.USER_GETBADGE);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.USER_GETBADGE);
