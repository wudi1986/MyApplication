...
public class GetByIdTaskService extends Service{

	public GetByIdTaskService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		// TODO Auto-generated constructor stub
		this.url = UrlParams.IP + Contanst.HTTP_GETBYIDTASK+urlParams;
	...
	void httpSuccess(String reponse) {
		...
		try {
			...
			// reponse =
			// ========================联调接口时注释就可以============================
			Tools.getLog(Tools.i, "bbb", "GetByIdTaskService  reponse = "+reponse);
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				...
				Gson gson = new Gson();
				ByIdDetailBean bean =  gson.fromJson(str, ByIdDetailBean.class);
				serviceListener.getJOSNdataSuccess(bean, retcode , Contanst.GETBYIDTASK);
			} else {
				String errmsg = (String) result.get("message");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.GETBYIDTASK);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.GETBYIDTASK);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.GETBYIDTASK);
