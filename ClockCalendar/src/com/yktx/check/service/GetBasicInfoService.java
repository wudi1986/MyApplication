...
public class GetBasicInfoService extends Service{

	public GetBasicInfoService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		// TODO Auto-generated constructor stub
		this.url=UrlParams.IP+Contanst.HTTP_GETBASICINFO+urlParams;
	...
	void httpSuccess(String reponse) {
		...
		try {
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				...
				Gson gson = new Gson();
				BasicInfoBean basicInfoBean = gson.fromJson(message, BasicInfoBean.class);
				serviceListener.getJOSNdataSuccess(basicInfoBean, retcode , Contanst.GETBASICINFO);
			} else {
				String errmsg = (String) result.get("message");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.GETBASICINFO);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.GETBASICINFO);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.GETBASICINFO);
